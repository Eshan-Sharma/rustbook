fn main() {
    ...
    ownership();
    length_example_with_ownership();
    length_example_with_reference();
}
...
fn length_example_with_ownership() {
    let s1 = String::from("hello");
    let (s2, len) = calculate_length(s1);
    println!("s2 is: {} with size: {}", s2, len);
    fn calculate_length(string_value: String) -> (String, usize) {
        let length = string_value.len();
        (string_value, length)
    }
}
fn length_example_with_reference() {
    let s1 = String::from("hello");
    ...
    fn calculate_length(s: &String) -> usize {
        s.len()
    } //s goes out of scope but it does not have ownership so the value is not dropped only reference is dropped
}
